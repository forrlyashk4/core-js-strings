...
function isString(value) {
  if (typeof value === 'object') {
    const testing = value;
    return testing instanceof String;
  }
...
function removeFirstOccurrences(str, value) {
  ...
  if (valueInd === -1) return str;
  return str.substring(0, valueInd) + str.substring(valueInd + value.length);
}
...
function isPalindrome(str) {
  const newStr = str.replaceAll(/[ ,!?]/g, '').toLowerCase();
  let mid = newStr.length / 2;
  if (!Number.isInteger(mid)) {
    const str1 = newStr.substring(0, mid);
    const str2 = newStr
      .substring(mid + 1, newStr.length)
      .split('')
      .reverse()
      .join('');
  ...
  mid = Math.round(mid);
  const str1 = newStr.substring(0, mid - 1);
  const str2 = newStr
    .substring(mid + 1, newStr.length)
    .split('')
    .reverse()
...
function findLongestWord(sentence) {
  ...
  sentence.split(' ').forEach((word) => {
    if (word.length > high.length) high = word;
  });
  return high;
}
...
function extractNameFromTemplate(value) {
  ...
  const lastName = wordsArr[2].split('!')[0];

  return `${wordsArr[1]} ${lastName}`;
}
